(function () {
    const __Mcop = function (scope) {
        ...
        this.utils = {
            ...
            removeOriginalHostAndGetRelativeUrl: function (url) {
                ...
                return fullURL.pathname + searchPart + fullURL.hash;
            },
            getBackOriginalUrl: function(targetedUrl){
                if (typeof targetedUrl !== 'string' || ! this.urlContainsOriginalHost(targetedUrl))
                    return targetedUrl;

                let protocol = (/^(http|https|ws|wss):\/\/.+/.test(targetedUrl)) ? null : 'https:';
                if (protocol === null) {
                    try {
                        const urlObjt = new URL(targetedUrl);
                        protocol = urlObjt.protocol;
                    } catch (e) {
                        return targetedUrl;
                    }
                }

                return protocol + '//' + this.extractOriginalHost(targetedUrl) + this.removeOriginalHostAndGetRelativeUrl(targetedUrl);
            },
    ...
    __Mcop.prototype.getRealUrl = function () {
        ...
        if (baseElt !== null) {
            if (this.scope.frameElement) {
                const originalSrc = this.utils.getBackOriginalUrl(this.scope.frameElement.src);
                this.scope.frameElement.setAttribute(this.utils.originalValueOf + this.utils.ucFirst('src'), originalSrc);
                baseElt.setAttribute('href', this.scope.frameElement.src);
                baseElt.setAttribute(this.utils.mcopHrefAttrib, originalSrc);
            }
    ...
    __Mcop.prototype.modifyUrl = function (oldUrl) {
        ...
        let newUrl = (oldUrl.includes('#')) ? oldUrl : oldUrl.replace(/\s*/mg, '').replace(/\?/g, '&').replace(/&/, '?');
        ...
        if (! this.isAbsoluteUrl(oldUrl)) {
            if (/^\/\//.test(oldUrl)) {
                ...
                if (typeof realHost === 'string' && realHost.length > 0 && ! oldUrl.includes('original-host')) {
        ...
        if (this.isAbsoluteUrl(oldUrl)) {
            ...
            newUrl = targetedUrlObjt.pathname;
        }
    ...
    __Mcop.prototype.getWindowIframe = function (targetedWindow) {
        const iframes = targetedWindow.parent.document.querySelectorAll('iframe');
        let iframeFound = null;
        iframes.forEach(function (currentNode, key, parent) {
            if (currentNode.contentWindow === targetedWindow) {
                iframeFound = currentNode;
            }
        });
    ...
    __Mcop.prototype.initScope = function () {
        ...
        if (this.scope.XMLHttpRequest && this.scope.XMLHttpRequest.prototype) {
            ...
            if (this.scope.XMLHttpRequest &&
                this.scope.XMLHttpRequest.prototype) {
                //We modify the responseURL property on XMLHttpRequest.prototype
                this.modifyProperty(this.scope.XMLHttpRequest.prototype, 'responseURL', function (descriptor) {
                    const modifiedUrl = thisProxy.modifyUrl(descriptor.get.call(this));
                    return thisProxy.utils.getBackOriginalUrl(modifiedUrl);
                });
            }
        }
        ...
        if (! this.scope[preparePostMsgData]) {
            this.scope[preparePostMsgData] = function(originalMsg) {
                let __msg = originalMsg;
                if (typeof originalMsg === 'object') {
                    __msg = JSON.stringify(originalMsg);
                }

                __msg = __msg.replace(/__mcopLocation/mg, 'location').replace(/__mcopOriginalFetch/mg, 'fetch');
                __msg = __msg.replace(/__mcopOriginalOpen/mg, 'open').replace(/__mcopOriginalSharedWorker/mg, 'SharedWorker');
                __msg = __msg.replace(/__mcopOriginalWebSocket/mg, 'WebSocket').replace(/__mcopOriginalWorker/mg, 'Worker');
                __msg = __msg.replace(/__mcopUtils\s*,/mg, '').replace(/_mcopPreparePostMessageMsg\s*,/mg, '');
                __msg = __msg.replace(/_mcopPreparePostMessageOrigin\s*,/mg, '');
    ...
    __Mcop.prototype.initWindow = function () {
        ...
        class McopElement {
                ...
                return {
                    ...
                    'iframe': {
                        modify: function () {
                           ...
                           const name = 'src';
                           const originalValueOfSrcAttr = thisProxy.utils.originalValueOf + thisProxy.utils.ucFirst(name);

                            if (thisElement.hasAttr(name)) {
                                const value = thisElement.getAttr(name);
                                thisElement.setAttr(originalValueOfSrcAttr, thisProxy.utils.getBackOriginalUrl(value));
            ...
            targetedAttribIsCorrect() {
                const targetedAttribs = {
                    src: 'src', href: 'href', data: 'data', action: 'action'
                };

                for (let name in targetedAttribs) {

                    if (this.hasAttr(targetedAttribs[name]) && thisProxy.isAbsoluteUrl(this.getAttr(targetedAttribs[name]))) {
                        //return (this.getAttr(targetedAttribs[name]) + '').startsWith(thisProxy.scope[thisProxy.utils.locationName].origin);
                        const secureOrigin = 'https://' + top.location.host;
                        const insecureOrigin = 'http://' + top.location.host;
                        ...
                        if (this.isIFrame()) {
                            ...
                            return value.startsWith(secureOrigin) || value.startsWith(insecureOrigin);

                        } else if (this.getTagName().toLowerCase() === 'img') {
                            const name = 'src';
                            const value = this.getAttr(targetedAttribs[name]) + '';
        ...
        this.modifyProperty(this.scope.Document.prototype, 'cookie', function (descriptor) {
            let currentCookies = descriptor.get.call(this) + '';

            if (/;/mg.test(currentCookies)) {
                const cookiesArray = currentCookies.split(/\s/gm);

                currentCookies = '';
                for (let i = 0; i < cookiesArray.length; i++) {
                    const mcopCookie = new McopCookie(cookiesArray[i]);
                    mcopCookie.extractNameAndValue();
                    let name = mcopCookie.getName();

                    if (typeof name === 'string' && name.length > 0) {
                        name = name.replace(/@.+/, '');
                        let value = mcopCookie.getValue();
        ...
        }, function (descriptor, rawCookie) {
            const cookie = new McopCookie(rawCookie);
            cookie.extractNameAndValue();
            if (typeof rawCookie === 'string' && typeof cookie.getName() === 'string' && ! cookie.getName().includes('@')) {
                const domainRegexp = /.+\..+/;
                let domain = cookie.getAttribute('domain');
                if (! domainRegexp.test(domain)) domain = thisProxy.getRealHost();
                const samesite = cookie.getAttribute('samesite');
                const path = cookie.getAttribute('path');
                const expires = cookie.getAttribute('expires');
                const maxAge = cookie.getAttribute('max-age');

                let name = (typeof cookie.getName() === 'string' && cookie.getName().length > 0) ? cookie.getName() : '';

                if (typeof domain === 'string' && ! /MCOP-SWREADY|SEOCROMOM/.test(name)) {
                ...
                if (/secure/.test(rawCookie))
                    cookieStr += 'secure';
                descriptor.set.call(this, cookieStr);
            } else {
                descriptor.set.call(this, rawCookie);
            }
        });
                    ...
                    mutation.addedNodes.forEach(function (currentNode) {
                        if(currentNode instanceof thisProxy.scope.Element) {
                            try{
                                ...
                                if (mcopElt.isTargetedTag() && ! mcopElt.targetedAttribIsCorrect()) {
    ...
    __Mcop.prototype.initLocation = function () {
        ...
        Object.defineProperty(McopLocation.prototype, 'href', {
            ...
            set: function (newHref) {
                ...
                if (! thisProxy.isTopWindow()) {
                    const windowIframe = thisProxy.getWindowIframe(thisProxy.scope);
                    if (windowIframe) {
                        windowIframe.src = modifiedHref;
                        return;
                    }
                }
        ...
        Object.defineProperty(McopLocation.prototype, 'protocol', {
            ...
            configurable: true,
            get: function () {
                return (this.expectedUrl !== thisProxy.utils.aboutBlank) ? this.URL.protocol : thisProxy.utils.aboutBlank;
            },
