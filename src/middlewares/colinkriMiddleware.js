...
const { JSON_to_URLEncoded, colinkriAuthLogin } = require("../services/utils");
    ...
    onProxyRes: responseInterceptor(
        async (responseBuffer, proxyRes, req, res) => {
            ...
            if (req.path == "/do-auto-login") {
                try {
                    let { username, password } = await credentialModel.findOne({"type": "colinkri"});
                    let result = await colinkriAuthLogin(username, password);
                    if (result) {
                        res.statusCode = 200;
            ...
            if (proxyRes.headers["content-type"] && proxyRes.headers["content-type"].includes("text/html")) {
                ...
                if (req.path == "/amember/login") {
                    let html = fs.readFileSync(__dirname + "/../views/colinkri-auth.ejs");
