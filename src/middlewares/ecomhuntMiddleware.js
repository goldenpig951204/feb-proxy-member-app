...
const ecomhuntMiddleware = (prefix) => createProxyMiddleware({
    target: `https://app.ecomhunt.com`,
    ...
    onProxyRes: responseInterceptor(
        async (responseBuffer, proxyRes, req, res) => {
            ...
            if (proxyRes.headers["location"]) {
                proxyRes.headers["location"] = proxyRes.headers["location"].replace("https://app.ecomhunt.com", domain);
                res.setHeader("location", proxyRes.headers["location"].replace("https://app.ecomhunt.com", domain));
            }
            if (proxyRes.headers["content-type"] && proxyRes.headers["content-type"].includes("text/html")) {
                ...
                links.each(function() {
                    ...
                    if (href !== undefined) {
                        $(this).attr("href", href.replace("https://app.ecomhunt.com", domain));
                    }
                });
                let forms = $("form");
                forms.each(function() {
                    ...
                    if (action !== undefined) {
                        $(this).attr("action", action.replace("https://app.ecomhunt.com", domain));
                    }
                });
                let anchors = $("a");
                ...
                scripts.each(function() {
                    ...
                    if (src == undefined) {
                        ...
                        let newContent = content.replaceAll("window.location.href = 'app.ecomhunt.com/keywords'", "window.location.href = 'https://payments.seozoom.it/keywords'");
                        newContent = newContent.replace(/https:\/\/app.ecomhunt.com/g, domain);
                        $(this).html(newContent);
                    } else {
                        $(this).attr("src", src.replace("https://app.ecomhunt.com", domain));
                    }
                });
                ...
                if (/\/login.php/.test(req.path)) {
                    let html = fs.readFileSync(__dirname + "/../views/ecomhunt-auth.ejs");
    ...
    hostRewrite: true,
    headers: {
        referer: "https://app.ecomhunt.com",
        origin: "https://app.ecomhunt.com"
    },
    autoRewrite: true,
    ws: true,
    timeout: 60000
...
module.exports = ecomhuntMiddleware;