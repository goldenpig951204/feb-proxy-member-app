...
const { JSON_to_URLEncoded, textoptimizerAuthLogin } = require("../services/utils");
    ...
    onProxyRes: responseInterceptor(
        async (responseBuffer, proxyRes, req, res) => {
            ...
            if (req.path == "/do-auto-login") {
                try {
                    let { username, password } = await credentialModel.findOne({"type": "textoptimizer"});
            ...
            if (proxyRes.headers["location"]) {
                proxyRes.headers["location"] = proxyRes.headers["location"].replace("https://textoptimizer.com", domain);
                res.setHeader("location", proxyRes.headers["location"].replace("https://textoptimizer.com", domain));
            }
            if (proxyRes.headers["content-type"] && proxyRes.headers["content-type"].includes("text/html")) {
                ...
                links.each(function() {
                    ...
                    if (href !== undefined) {
                        $(this).attr("href", href.replace("https://textoptimizer.com", domain));
                    }
                });
                let forms = $("form");
                forms.each(function() {
                    ...
                    if (action !== undefined) {
                        $(this).attr("action", action.replace("https://textoptimizer.com", domain));
                    }
                });
                let anchors = $("a");
                ...
                scripts.each(function() {
                    ...
                    if (src == undefined) {
                        ...
                        let newContent = content.replace(/https:\/\/textoptimizer.com/g, domain);
                        $(this).html(newContent);
                    } else {
                        $(this).attr("src", src.replace("https://textoptimizer.com", domain));
                    }
                });
                ...
                if (req.path == "/login") {
                    let html = fs.readFileSync(__dirname + "/../views/textoptimizer-auth.ejs");
