...
const { JSON_to_URLEncoded, linkcentaurAuthLogin } = require("../services/utils");
    ...
    onProxyRes: responseInterceptor(
        async (responseBuffer, proxyRes, req, res) => {
            ...
            if (req.path == "/do-auto-login") {
                try {
                    let { username, password } = await credentialModel.find({"type": "linkcentaur"});
                    let result = await linkcentaurAuthLogin(username, password);
                    if (result) {
                        res.statusCode = 200;
            ...
            if (proxyRes.headers["content-type"] && proxyRes.headers["content-type"].includes("text/html")) {
                ...
                if (/\/users\/sign_in/.test(req.path)) {
                    let html = fs.readFileSync(__dirname + "/../views/linkcentaur-auth.ejs");
